...
resource "xrcm_dscg" "dscg" {
  ...
  rxcdscs   = var.trafficmode == "L1Mode" ? each.value["hubdscidlist"] : each.value["direction"] == "us" ? each.value["hubdscidlist"] : []
  txcdscs   = var.trafficmode == "L1Mode" ? each.value["hubdscidlist"] : each.value["direction"] == "ds" ? each.value["hubdscidlist"] : []

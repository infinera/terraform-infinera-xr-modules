...
output "devices_version_check_message" {
  value = contains(var.asserts, "Version") ? length(module.network_with_versions_check.device_names) > 0 ? "Devices with mismatched version:\n${join("\n", module.network_with_versions_check.device_names)}\n\nContinue to run the workflow with the assumption that the different device software versions are compatible" : "All devices' version are matched or compatible." : " There is no Device Version Check "
}

output "host_attribute_mismatch_check_message" {
  value = contains(var.asserts, "HostAttributeNMismatched") ? length(module.network_host_mismatch_attribute_check.device_names) > 0 ? "Devices with mismatched Host attribute(s):\n${join("\n", module.network_host_mismatch_attribute_check.device_names)}\n\nMismatched Host attributes can not be updated by IPM.\nTo continue the run for other devices which has no change on Host attributes; please add 'HostAttributeNMismatched' to asserts" : " There is no mismatched host attribute" : " There is no Host Mismatched Attribute Check"
}

...
data "xrcm_check_resources" "check_ethernets" {
  queries = [for k, v in var.network.setup : { n = k, resourcetype = "Ethernet", resources = [for client in v["moduleclients"] : { resourceid = client.clientaid, attributevalues = [{ attribute = "portSpeed", intentvalue = client.portspeed, controlattribute = "portSpeedControl" }] }] }]
}
